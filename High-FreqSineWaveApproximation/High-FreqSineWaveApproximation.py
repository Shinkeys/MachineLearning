...
class MLP(nn.Module):
    """
    MLP with Fourier Features for approximating high-frequency sine waves:
    - Input: 1 neuron
    - Fourier Feature Encoding: transforms input to sin/cos features
    - Hidden layer 1: 16 neurons with ReLU
    - Hidden layer 2: 16 neurons with ReLU
    - Output: 1 neuron (linear)
    """

    def __init__(self, num_fourier_features=6):
        super(MLP, self).__init__()
        
        self.num_fourier_features = num_fourier_features
        # Fourier feature frequencies (learnable or fixed)
        # Using fixed frequencies as powers of 2 for better coverage
        self.register_buffer('fourier_freqs', 
                            torch.FloatTensor([2**i for i in range(num_fourier_features)]))
        
        # Input dimension after Fourier encoding: 2 * num_fourier_features (sin + cos for each frequency)
        fourier_dim = 2 * num_fourier_features
        
        self.network = nn.Sequential(
            nn.Linear(fourier_dim, 16),    # Fourier features -> 16
            nn.ReLU(),                     # Activation
    ...
    def fourier_encoding(self, x):
        """
        Transform input x to Fourier features using sin/cos transformations
...
def generate_data(n_train=2048, n_val=256, x_min=-np.pi, x_max=np.pi, frequency=10):
    """Generate training and validation data as PyTorch tensors for high-frequency sine wave"""
    ...
    y_train = torch.FloatTensor(np.sin(frequency * x_train.numpy()))
    y_val = torch.FloatTensor(np.sin(frequency * x_val.numpy()))
    return (x_train, y_train), (x_val, y_val)


def visualize_realtime_training(train_data, val_data, epochs=1000, learning_rate=1e-2, batch_size=64, frequency=10, num_fourier_features=10):
    x_train, y_train = train_data
    x_val, y_val = val_data

    mlp = MLP(num_fourier_features=num_fourier_features)
    ...
    y_true_smooth = np.sin(frequency * x_smooth_np)

    mlp.eval()
    with torch.no_grad():
        y_init = mlp(x_smooth).numpy().ravel()
    
    line_true, = ax.plot(x_smooth_np, y_true_smooth, 'b-', linewidth=2, label=f'True sin({frequency}x)', alpha=0.8)
    ...
    ax.set_xlabel('x')
    ax.set_ylabel(f'sin({frequency}x)')
    ax.set_title(f'Real-time MLP High-Frequency Sine Wave Approximation\n(frequency={frequency}, Fourier features={num_fourier_features})')
...
if __name__ == "__main__":
    ...
    frequency = 32  # High-frequency sine wave multiplier
    num_fourier_features = 5  # 2 pow 5 = 32 which is the number of frequency
    
    train_data, val_data = generate_data(n_train=512, n_val=128, frequency=frequency)
    visualize_realtime_training(train_data, val_data, epochs=1000, learning_rate=1e-2, batch_size=64, 
                               frequency=frequency, num_fourier_features=num_fourier_features)